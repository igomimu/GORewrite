# 本日の開発タスク: Snap Goban（棋譜再生・印刷アプリ）枝分かれ手順実装
...
- [x] SVGエクスポート時の白黒反転問題の調査（20:00前の事前チェック完了）
    - [x] 原因：純粋な黒(#000000)/白(#FFFFFF)がダークモード環境で自動反転の対象になっている可能性
    - [x] 対策案：オフカラー(#000001, #FEFEFE)の使用と`<g>`ラッピングによるスタイル保護
- [x] 現状のデータ構造の確認
...
- [ ] スキーマの更新（必要な場合）
    ...
    - [ ] 再帰的構造またはParent参照の導入
    - **Note:** Snap Goban拡張機能としては実装完了済み。Sanity側の対応は保留。
