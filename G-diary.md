...
- **2026-01-20**:
    - **[Feature] Area Move (Drag & Drop)**:
        - **範囲移動**: 選択範囲（青枠）内をドラッグすることで、その範囲内の石をまとめて移動できるようになりました。
        - **仕様**: 
            - 選択範囲を作成後、範囲内でマウスを押下してドラッグすると移動モードになります。
            - 移動先に石がある場合は上書きされます。
            - 移動先が範囲外になる場合、その石は削除されます。
    - **[Fix] Version Mismatch (v2.0.1)**:
        - ストア公開後に「機能一覧では2.0.0だがサイドバー表記が1.6.1のまま」という現象が発生。
        - 原因: ビルドアーティファクト(dist)の更新漏れ、または環境不一致。
        - 対応: バージョンを **v2.0.1** に更新し、クリーンビルドを実施。正しいバージョン表記を含む `GORewrite_v2.0.1.zip` を生成しました。
    - **[Fix] Version Bump (v2.0.2)**:
        - ユーザーより「v2.0.1 は既に公開済み」との指摘があったため、バージョン競合を避け、かつ確実に表示不具合を解消するために **v2.0.2** へ更新しました。
        - 変更点はバージョン番号の更新のみです。再ビルドにより正しいバージョン表記(v2.0.2)がサイドバーに表示されることを保証します。

- **2026-01-26**:
    - **[Feature] Printing Overhaul (v2.0.5)**:
        - **Direct Print**: 印刷ボタン押下時に新しいタブを開かず、サイドバー内で完結するように変更しました。
        - **No Preview Page**: プレビュー用のモード切り替え（ヘッダー表示等）を廃止し、エディタ画面からダイレクトに Windows 印刷ダイアログが開くように改良しました。
